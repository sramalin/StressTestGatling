...
class armDevice extends Simulation {
  ...
  val getAccessToken = scenario("Arm device")
      ...
      .exec(ArmObject.armDevice)


  object ArmObject {
    val authToken = "Bearer " + "${authToken1}"
  val armDevice =scenario("Arm Device")
                .exec{session =>session.set("data1","{'PartitionId':0}")}
                .exec(http("Arm device scenario")
      ...
      //.body(StringBody("""{ "Name": "Away","Data":"{ "PartitionId":":0"}" }""")).asJSON
                .body(StringBody("""{ "Name": "Away","Data":"${data1}" }""")).asJSON
                .check(status.is(200))
                .extraInfoExtractor {
                  extraInfo =>
